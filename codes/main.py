...
if __name__ == "__main__":
    ...
    if args.option == "AC":
        ...
        if os.path.exists(success + '/' + pdb1_name) and succ_dir_count >= 8:
            ...
            pred_1st_all = prediction_all_AC_inc(pdb1, pdb1_name, pdb2, pdb2_name, search_dir)
            shallow_MSA_size = []
            shallow_MSA_size = np.append(shallow_MSA_size, pred_1st_all.size_selection)
            print("               ")
            print("Specific size of shallow random MSA is similar to full-MSA")
        ...
        if os.path.isdir( add_pred_dir ):
            ...
            if num_files > 200:
                ...
                additional_prediction_AC_inc(search_dir, pdb1, pdb1_name, pdb2, pdb2_name, pwd, shallow_MSA_size)
        ...
        ######################################################################################################
        ##### plot the 2D-scatter plot of TM-scores with pLDDT
        plot_2D_scatter_AC_inc(full, random, addition, pdb1, pdb1_name, pdb2, pdb2_name)
