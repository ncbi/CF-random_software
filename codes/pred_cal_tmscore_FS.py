...
class CF_MSA_var():
    ...
    def select_size(self, TMscores_random_alter, TMscores_fs_random_alter, pdb1_name, pdb2_name, alt_name):
        ...
        #if (np.any(TMscore_data[location, :] >= 0.5) and np.any(TMscore_fs_data[location, :] >= 0.5)):
        #if alt_name == pdb2_name and ((np.any(TMscore_data[location, :] >= 0.5) and np.any(TMscore_fs_data[location, :] >= 0.5)) or (np.any(TMscore_data[location - 1, :] >= 0.5) and np.any(TMscore_fs_data[location - 1, :] >= 0.5))):
        ...
        #elif alt_name == pdb1_name and ((np.any(TMscore_data[location, :] >= 0.5) and np.any(TMscore_fs_data[location, :] >= 0.5)) or (np.any(TMscore_data[location + 1, :] >= 0.5) and np.any(TMscore_fs_data[location + 1, :] >= 0.5))):
        ...
        #TMscore_data = TMscores_random_alter
        #TMscore_data = TMscores_random_alter.reshape(7, 5)
        #TMscore_data_sum = np.zeros((7, 1))


        #TMscore_fs_data = TMscores_fs_random_alter
        #TMscore_fs_data = TMscores_fs_random_alter.reshape(7, 5)
        #
        #print((TMscore_data.shape[0]))
        #print((TMscore_fs_data.shape[0]))
        ...
        ##    print("Predictions are bad")
        ##    rm_folder_cmd = 'rm -rf successed_prediction/' + self.pdb1_name + '/'
        ##    print(rm_folder_cmd); os.system(rm_folder_cmd)
        ##    sys.exit()
        ...
        #for jj in range(0, int(TMscore_data.shape[0])):
        #    print(TMscore_data[jj, :], TMscore_fs_data[jj, :])
        #    if np.any(TMscore_data[jj, :] > 0.5) and np.any(TMscore_fs_data[jj, :] > 0.5):
        #        selection = jj
        #        self.selection = selection
        #        break
...
class prediction_all():
    def __init__(self, pdb1, pdb1_name, pdb2, pdb2_name, search_dir):
        ...
        #elif np.average(fs_TMscore[1, :]) > np.average(fs_TMscore[0, :]):
        ...
        #else:
        #    print("Performing predictions with full-MSA using additional 15 seeds")
        #    #additional command for running more full-length MSA
        #    MSA_full_add = MSA_full.additional_max(search_dir, output_dir,random_seed, pdb1, pdb1_name, pdb2, pdb2_name, pwd)
        #    ref_name = MSA_full_add.ref_name; alt_name = MSA_full_add.alt_name
