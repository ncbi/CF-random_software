...
class CF_MSA_max():
    def __init__(self, search_dir, output_dir, pdb_name, rseed, num_seeds):

        command = 'colabfold_batch --num-seeds ' + str(num_seeds) + ' --random-seed ' + str(rseed) + search_dir + output_dir
        print(command)
        os.system(command)
        
        
    def additional_max(self, search_dir, output_dir, rseed, pdb1, pdb1_name, pdb2, pdb2_name, pwd, num_seeds):
        ...
        for add_rseed in add_trial:
            ...
            MSA_full = CF_MSA_max(search_dir, output_dir, pdb1_name, add_rseed, num_seeds)
            ...
            if np.any(fs_TMscore[0, :] >= 0.5) and np.any(full_TMscore[0, :] >= 0.5):
                ...
                print(mv_folder_cmd_suc); os.system(mv_folder_cmd_suc)
...
class CF_MSA_var():
    def __init__(self, pdb1, pdb1_name, pdb2, pdb2_name, search_dir, output_dir, rseed, ref_name, alt_name, num_seeds):
        #### shallow MSA section
        #### Global viarlable
        max_msa = 1
        ext_msa = 2
        ...
        for ran_seed in random_seed:
            ...
            for multi in (1, 2, 2, 2, 2, 2, 2):
                ...
                #### Colabfold part
                command = 'colabfold_batch --num-seeds ' + str(num_seeds) + ' --max-seq ' + str(max_msa) + ' --max-extra-seq ' + str(ext_msa) + search_dir + output_dir + str(ran_seed) + '_max_' + str(max_msa) + '_ext_' + str(ext_msa)
                print(command)
                os.system(command)
            ...
            TMscores_random_reshape = self.TMscores_random.reshape(14, num_seeds * 5)
            TMscores_fs_random_reshape = self.TMscores_fs_random.reshape(14, num_seeds * 5)
            ...
            if np.any(TMscores_random_reshape > 0.5) and np.any(TMscores_fs_random_reshape > 0.5):
                ...
                if not os.path.exists(gen_dir):
                    ...
                    #self.TMscores_fs_random_alter = TMscores_fs_random_alter
    ...
    def select_size(self, TMscores_random_alter, TMscores_fs_random_alter, pdb1_name, pdb2_name, alt_name, num_seeds):
        ...
        if alt_name == pdb2_name and ((np.any(TMscore_data[location, :] >= 0.5) and np.any(TMscore_fs_data[location, :] >= 0.5))):
            ...
            for jj in range(0, int(TMscore_data.shape[0] / 2)):
                ...
                #if (np.any(TMscore_data[(jj * 2), :] >= 0.45) and np.any(TMscore_fs_data[(jj * 2) + 1, :] >= 0.5)) or (np.any(TMscore_data[(jj * 2) + 1, :] >= 0.45) and np.any(TMscore_fs_data[(jj * 2), :] >= 0.5)):
                if (np.any(TMscore_data[(jj * 2), :] >= 0.4) and np.any(TMscore_fs_data[(jj * 2) + 1, :] >= 0.5)) or (np.any(TMscore_data[(jj * 2) + 1, :] >= 0.4) and np.any(TMscore_fs_data[(jj * 2), :] >= 0.5)):
                    ...
                    self.selection = selection
                    break
                #elif (np.any(TMscore_data[(jj * 2), :] >= 0.45) and np.any(TMscore_fs_data[(jj * 2), :] >= 0.5)) or (np.any(TMscore_data[(jj * 2) + 1, :] >= 0.45) and np.any(TMscore_fs_data[(jj * 2) + 1, :] >= 0.5)):
...
class prediction_all():
    def __init__(self, pdb1, pdb1_name, pdb2, pdb2_name, search_dir, nMSA):
        num_seeds = 5 + nMSA
        ...
        MSA_full = CF_MSA_max(search_dir, output_dir, pdb1_name, random_seed, num_seeds)
        ...
        fs_TMscore = np.array(MSA_fs_TMscore.tmscores_fs)
        fs_TMscore = fs_TMscore.reshape(2, num_seeds * 5)
        #print(fs_TMscore[0, :]) ;         #print(fs_TMscore[1, :])
        ...
        if np.average(full_TMscore[0, :]) > np.average(full_TMscore[1, :]):
            if np.any(fs_TMscore[0, :] >= 0.5) and np.any(full_TMscore[0, :] >= 0.5):
                ...
                MSA_full_add = MSA_full.additional_max(search_dir, output_dir,random_seed, pdb1, pdb1_name, pdb2, pdb2_name, pwd, num_seeds)
                ref_name = MSA_full.ref_name  ; alt_name = MSA_full.alt_name
        ...
        MSA_var = CF_MSA_var(pdb1, pdb1_name, pdb2, pdb2_name, search_dir, output_dir, random_seed, ref_name, alt_name, num_seeds)


        print("                                     ")
        print("Finding optimal size of ramdon MSA...")
        MSA_var.select_size(MSA_var.TMscores_random_alter, MSA_var.TMscores_fs_random_alter, pdb1_name, pdb2_name, alt_name, num_seeds)
