...
class plot_2D_scatter_AC():
    def __init__(self, full_cate, random_cate, addition_cate, pdb1, pdb1_name, pdb2, pdb2_name):
        ...
        TMs_full_resh = np.reshape(TMs_full, (10, 5))

        f1 = np.concatenate((TMs_addition[0:20, :], TMs_full_resh[0:5, :]), axis=0)
        print(f1)
        f2 = np.concatenate((TMs_addition[20:40, :], TMs_full_resh[5:10, :]), axis=0)
        print(f2)
